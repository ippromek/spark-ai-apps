/**
  ...
  * https://github.com/ippromek/spark-ml/blob/branch-ed2/Chapter_11/2.0.0/scala-spark-streaming-app/src/main/scala/org/sparksamples/Streaming.scala
  */
...
object StreamingProducer {
  ...
  def main(args: Array[String]) {
    ...
    //println("QWE "+this.getClass.getClassLoader)
   // val namesResource = this.getClass.getClassLoader.fromResource("C:\\names.csv")
   val namesResource = this.getClass.getClassLoader.getResourceAsStream("names.csv")
    //println("namesResource "+namesResource)

  //  val namesResource=Source.fromInputStream(getClass.getResourceAsStream("C:\\names.csv"))
    ...
    while (true) {
      ...
      new Thread() {
        override def run = {
          ...
          while (true) {
            Thread.sleep(1000)
            val num = random.nextInt(MaxEvents-1)+1
