...
object StumbleUponExecutor {
  ...
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("StumbleUpon").setMaster("local[1]")
    ...
    val df = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", "\t").option("header", "true")
      .option("inferSchema", "true").load("C:\\Users\\oleg.baydakov\\IdeaProjects\\SparkMachineLearning2edition\\src\\main\\resources\\train.tsv")
